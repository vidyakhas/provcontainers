...
CALL "%INSTALLUTIL%" /u "%SERVICE_INSTALL%\PredixMachineService.exe"
IF %ERRORLEVEL% NEQ 0 (
	ECHO Predix Machine Service uninstallation failed with an error. View the InstallUtil.InstallLog and PredixMachineService.InstallLog for more details.
) ELSE (
	ECHO Predix Machine Service uninstalled successfully.
)